# Understanding Module Descriptors — SETU CPD

**Module 1** of the SETU Continuing Professional Development series for
part-time and industry-based lecturers. A clean, modern, self-paced SCORM 1.2
course built as a **standalone static site** — no build step, no server
framework, no external dependencies (brand fonts are self-hosted, JS is vanilla).

Built from the *Understanding Module Descriptors — Full Narration & Build Script
for Articulate Rise On-Demand Session* (Script 1, V2), and styled to the **SETU
Online Course Brand Guide** (SETU Brand Guidelines v1, May 2022) and the
*Lifelong Learning at SETU Design Toolkit*.

Derived from [`kenmccarthy/cpd-course-template`](https://github.com/kenmccarthy/cpd-course-template),
which carries the reusable shell (navigation, Rise layer, SCORM plumbing,
animations, accessibility conventions). Fixes to that shell are worth porting
back to the template so later modules inherit them.
...
|---|---|---|
| 1 | Understanding Module Descriptors | this repo |
| 2 | Teaching Online | [`cpd-course-module02`](https://github.com/kenmccarthy/cpd-course-module02) |

The shell is shared; each module keeps its own `storeKey`, so a learner's
progress in one never collides with another even when both are served from the
same origin.
...
The course follows the Articulate Rise pattern and the *Lifelong Learning at SETU
...
The course is built to WCAG 2.1 AA and audited with axe-core (no violations
